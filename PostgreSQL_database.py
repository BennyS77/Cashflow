...
with engine.connect() as conn:
    conn.execute(text("CREATE TABLE some_table (x int, y int, z int)"))
    conn.execute(
...
stmt = text("SELECT x, y, z FROM some_table")
with Session(engine) as session:
    result = session.execute(stmt)
    for row in result:
        #  print(f"x: {row.x}  y: {row.y}")
         st.write(f"x: {row.x}  y: {row.y}  z: {row.z}")


newValue = st.text_input("input")
st.write(newValue)
